...
from competition_service.application.repositories.match_lineup_repository import MatchLineupRepository


class PostgresMatchLineupRepository(MatchLineupRepository):
    """Postgres adapter stub. Never treats a stub call as a successful write."""

    def __init__(self, connection_url: str) -> None:
        self.connection_url = connection_url

    async def create(self, entity: MatchLineup) -> MatchLineup:
        raise NotImplementedError("Transactional create and serialization")

    async def get(self, entity_id: UUID) -> MatchLineup | None:
        raise NotImplementedError("Read by internal ID")

    async def list(self, *, limit: int = 50, offset: int = 0) -> tuple[MatchLineup, ...]:
        raise NotImplementedError("Bounded query")

    async def update(self, entity: MatchLineup, *, expected_version: int) -> MatchLineup:
